...
def create_wall_straight(col, mat_stone):
    ...
    for i in range(layer_count):
        ...
        # Buat faces
        bm.faces.new([verts[0], verts[1], verts[5], verts[4]])  # depan
        bm.faces.new([verts[2], verts[3], verts[7], verts[6]])  # belakang
        bm.faces.new([verts[1], verts[2], verts[6], verts[5]])  # kanan
        bm.faces.new([verts[3], verts[0], verts[4], verts[7]])  # kiri
        bm.faces.new([verts[4], verts[5], verts[6], verts[7]])  # atas
        bm.faces.new([verts[3], verts[2], verts[1], verts[0]])  # bawah
...
def create_wall_corner(col, mat_stone):
    ...
    for i in range(layer_count):
        ...
        bm.faces.new([verts_x[0], verts_x[1], verts_x[5], verts_x[4]])
        bm.faces.new([verts_x[2], verts_x[3], verts_x[7], verts_x[6]])
        bm.faces.new([verts_x[1], verts_x[2], verts_x[6], verts_x[5]])
        bm.faces.new([verts_x[3], verts_x[0], verts_x[4], verts_x[7]])
        bm.faces.new([verts_x[4], verts_x[5], verts_x[6], verts_x[7]])
        bm.faces.new([verts_x[3], verts_x[2], verts_x[1], verts_x[0]])
        ...
        bm.faces.new([verts_y[0], verts_y[1], verts_y[5], verts_y[4]])
        bm.faces.new([verts_y[2], verts_y[3], verts_y[7], verts_y[6]])
        bm.faces.new([verts_y[1], verts_y[2], verts_y[6], verts_y[5]])
        bm.faces.new([verts_y[3], verts_y[0], verts_y[4], verts_y[7]])
        bm.faces.new([verts_y[4], verts_y[5], verts_y[6], verts_y[7]])
        bm.faces.new([verts_y[3], verts_y[2], verts_y[1], verts_y[0]])
...
def create_wall_end_cap(col, mat_stone):
    ...
    for i in range(layer_count):
        ...
        bm.faces.new([vl[0], vl[1], vl[5], vl[4]])
        bm.faces.new([vl[2], vl[3], vl[7], vl[6]])
        bm.faces.new([vl[1], vl[2], vl[6], vl[5]])
        bm.faces.new([vl[3], vl[0], vl[4], vl[7]])
        bm.faces.new([vl[4], vl[5], vl[6], vl[7]])
        bm.faces.new([vl[3], vl[2], vl[1], vl[0]])
        ...
        bm.faces.new([vr[0], vr[1], vr[5], vr[4]])
        bm.faces.new([vr[2], vr[3], vr[7], vr[6]])
        bm.faces.new([vr[1], vr[2], vr[6], vr[5]])
        bm.faces.new([vr[3], vr[0], vr[4], vr[7]])
        bm.faces.new([vr[4], vr[5], vr[6], vr[7]])
        bm.faces.new([vr[3], vr[2], vr[1], vr[0]])
...
def create_ceiling_arch(col, mat_cave):
    ...
    for i in range(segments):
        ...
        # Muka depan arch
        try:
            bm.faces.new([vbb[i], vbb[i+1], vtb[i+1], vtb[i]])
        except ValueError:
            pass

        # Sisi bawah / inner
        try:
            bm.faces.new([vbt[i], vtt[i], vtt[i+1], vbt[i+1]])
        except ValueError:
            pass

        # Sisi kiri / samping
        try:
            bm.faces.new([vbb[i], vbt[i], vtt[i], vtb[i]])
        except ValueError:
            pass

        # Sisi kanan / samping
        try:
            bm.faces.new([vbb[i+1], vtb[i+1], vtt[i+1], vbt[i+1]])
        except ValueError:
            pass
    
    # Tutup ujung-ujung arch
    bm.faces.new(vbb + [vbt[i] for i in range(len(vbt))][::-1])
    bm.faces.new(vtb + [vtt[i] for i in range(len(vtt))][::-1])
...
def create_fireplace(col, mat_brick, mat_wood, mat_stone, mat_ember):
    ...
    v_floor = [(-fp_w/2 + 0.25, 0, 0), (fp_w/2 - 0.25, 0, 0),
               (fp_w/2 - 0.25, fp_d, 0), (-fp_w/2 + 0.25, fp_d, 0)]
    bm.faces.new([bm.verts.new(c) for c in v_floor])
    ...
    for log_idx in range(3):
        ...
        bm.faces.new(end_verts_l)
        bm.faces.new(end_verts_r[::-1])
...
def export_blend(export_dir):
    """Simpan file .blend."""
    blend_path = os.path.join(export_dir, BLEND_FILENAME)
    bpy.ops.wm.save_as_mainfile(filepath=blend_path)
    print(f"[EXPORT] .blend disimpan: {blend_path}")


def export_fbx(export_dir):
    """
    Export semua aset sebagai FBX yang kompatibel dengan Roblox Studio.
    
    Setting FBX untuk Roblox:
    - axis_forward: -Z (Roblox menggunakan Y-up)
    - axis_up: Y
    ...
    # Select semua mesh object (kecuali camera, light, empty)
    ...
    bpy.ops.export_scene.fbx(
        filepath            = fbx_path,
        use_selection       = True,        # Hanya objek yang dipilih
        global_scale        = 1.0,
        apply_unit_scale    = True,
        apply_scale_options = 'FBX_SCALE_ALL',
        axis_forward        = '-Z',        # Roblox: Y-up
        axis_up             = 'Y',
        object_types        = {'MESH'},    # Hanya mesh
        use_mesh_modifiers  = True,        # Apply modifiers
        mesh_smooth_type    = 'FACE',
        use_triangles       = True,        # Roblox lebih suka triangles
        bake_space_transform = True,
        embed_textures      = False,       # Tidak ada texture external
        path_mode           = 'AUTO',
        use_metadata        = True,
    )
    
    print(f"[EXPORT] .fbx disimpan: {fbx_path}")
    bpy.ops.object.select_all(action='DESELECT')
...
def main():
    ...
    # 8. Export
    print("\n[EXPORT] Memulai export...")
    export_dir = ensure_export_dir()
    export_fbx(export_dir)
    export_blend(export_dir)
    
    print("\n" + "=" * 60)
    print("SELESAI! Asset pack berhasil dibuat.")
    print(f"Lokasi file: {export_dir}")
    print(f"  - {BLEND_FILENAME}")
    print(f"  - {FBX_FILENAME}")
    print("=" * 60)
